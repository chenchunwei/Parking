...
int InitializeCpuCard(char* snData) {
	U8 returnLen;
...
//初始化CPU卡 选择互连互通应用
int InitailizeAppOfCPU() {
	U8 returnLen;
	unsigned char returnBuffer[50];
	//00A4040009A00000000386980701
	char command[] = { 0x00, 0xA4, 0x04, 0x00, 0x09, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x86, 0x98, 0x07, 0x01 };
...
int InitailizePsamCard(int slot) {
	...
	int openReturn = ICC_PowerOpen(slot, 38400, 3, returnBuffer);
	if (openReturn == 0)
		return 1;
	return 0;
}
//读取PSAM卡编号 返回值9000 长度6字节 第二步  返回终端机编号
int ReadPsamNo(int slot, char* psamNo) {
	int returnLen;
	...
	if (returnValueOfCommand == 0x9000) {
		memcpy(psamNo, returnBuffer, 6);
		return 0;
	}
	return 1;
}

//PSAM卡选择文件 选择全国密钥管理总中心ADF  第三步  返回???
int SelectAdfSecretFromPsam(int slot, char* returnBuffer) {
	int returnLen;
	char command[] = { 0x00, 0xA4, 0x00, 0x00, 0x02, 0x10, 0x01 };
	unsigned short returnValueOfCommand = ICC_IsoCommand(slot, sizeof (command), command, &returnLen, returnBuffer);
	if (returnValueOfCommand == 0x6128)
		return 0;
	return 1;
}

//PSAMPSAM卡获取消费密钥索引:00B0970001，获取消费密钥索引【返回值9000 长度1 内容1】  第三步  返回消费密钥索引
int SelectSecretIndexFromPsam(int slot, char* secretIndex) {
	int returnLen;
	unsigned char returnBuffer[1];
	char command[] = { 0x00, 0xB0, 0x97, 0x00, 0x01 };
	unsigned short returnValueOfCommand = ICC_IsoCommand(slot, sizeof (command), command, &returnLen, returnBuffer);
	if (returnValueOfCommand == 0x9000) {
		memcpy(secretIndex, returnBuffer, 1);
		return 0;
	}
	return 1;
}
...
int ReadCardInfoOfCPU(char* city, char* appSn) {
	U8 returnLen;
...
//CPU 读取用户卡安全识别码 80CA000009，指令执行成功后获取9字节安全识别码【 返回9000 长度9】第四.1步
int ReadSecretCodeOfCPU(char* cpuSecretCode) {
	U8 returnLen;
	unsigned char returnBuffer[9];
...
//CPU 读取用户卡安全识别码 80CA000009，指令执行成功后获取9字节安全识别码【 返回9000 长度9】第四.2步
int ConfirmSecretCodeOfPSAM(int slot, char* cpuSecretCode) {
	U8 returnLen;
	unsigned char returnBuffer[9];
	char command[14] = { 0x80, 0xCA, 0x0, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	memcpy(command + 5, cpuSecretCode, 9);
	unsigned short returnValueOfCommand = ICC_IsoCommand(slot, sizeof (command), command, &returnLen, returnBuffer);
	if (returnValueOfCommand == 0x9000) {
		memcpy(cpuSecretCode, returnBuffer, 9);
		return 0;
 ...
 */
int InitialConsumeOfCPU(char* secretIndex, char* moneyHex, char* psamNo, char *returnlenHex, char* returnBalance, char* returnNo,
		char* returnOverdraft, char* returnSecretVer, char* returnAlgorithmVer, char* returnRandom) {
	U8 returnlen;
	...
	unsigned short returnValueOfCommand = ISO14443A_APDU(sizeof (command), command, &returnlen, returnBuffer);
	if (returnValueOfCommand == 0x9000) {
		memcpy(returnBalance, returnBuffer, 4);
		memcpy(returnNo, returnBuffer + 4, 2);
		memcpy(returnOverdraft, returnBuffer + 4 + 2, 3);
		memcpy(returnSecretVer, returnBuffer + 4 + 2 + 3, 1);
		memcpy(returnAlgorithmVer, returnBuffer + 4 + 2 + 3 + 1, 1);
		memcpy(returnRandom, returnBuffer + 4 + 2 + 3 + 1 + 1, 4);
		return 0;
	}
	return 1;
}
...
int CalcMac1InPsam(int slot, char* random, char* no, char* moneyHex, char* oderType, char* dateTime, char* secretVer, char* algorithmVer, char* appSn,
	...
	U8 returnlen;
	char returnBuffer[8];
	char command[36] = { 0x80, 0x70, 0x00, 0x00, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	unsigned short returnValueOfCommand = ICC_IsoCommand(slot, sizeof (command), command, &returnlen, returnBuffer);
	if (returnValueOfCommand == 0x6108) {
		char getResponseCmd[5] = { 0x00, 0xc0, 0x00, 0x00, 0x08 };
		returnValueOfCommand = ICC_IsoCommand(slot, sizeof (getResponseCmd), getResponseCmd, &returnlen, returnBuffer);
		if (returnValueOfCommand == 0x9000) {
 ...
 * */
int ConsumeCommandOfCPU(char* psamOderSn, char* dateTime, char* mac1, char* returnTac, char* returnMac2) {
	U8 returnlen;
	char returnBuffer[8];
	char command[21] =
			{ 0x80, 0x54, 0x01, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08 };
	//写入 4字节PSAM卡终端交易序号
	...
	unsigned short returnValueOfCommand = ISO14443A_APDU(sizeof (command), command, &returnlen, returnBuffer);
	if (returnValueOfCommand == 0x9000) {
		memcpy(returnTac, returnBuffer, 4);
		memcpy(returnMac2, returnBuffer + 4, 4);
		return 0;
	}
	return 1;
}
...
int CalcMac2InPsam(int slot, char* mac2) {
	...
	char command[] = { 0x80, 0x72, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00 };
	//写入mac2
...
int ReadBalanceCommandOfCPU(char* balance) {
	U8 returnlen;
	char returnBuffer[4];
...
int ReadBalanceInt2CommandOfCPU(int *money) {
	char moneyHex[4];
	int returnValue = ReadBalanceCommandOfCPU(moneyHex);
	if (returnValue == 0) {
		int returnState = HexToInt32(moneyHex, money);
		return 0;
	}
	return 1;
}
...
int ReadCityCommandOfCPU(char* city) {
	U8 returnlen;
	char returnBuffer[2];
	char command[] = { 0x00, 0xb0, 0x95, 0x02, 0x02 };
	unsigned short returnValueOfCommand = ISO14443A_APDU(sizeof (command), command, &returnlen, city);
	if (returnValueOfCommand == 0x9000) {
		return 0;
	}
	return 1;
...
int ReadAppSnCommandOfCPU(char* appSn) {
	U8 returnlen;
	char returnBuffer[20];
...
int ReadUserNameCommandOfCPU(char* name) {
	U8 returnlen;
	char returnBuffer[20];
...
int ReadUserIdCardNumberCommandOfCPU(char* userIdCardNumber) {
	U8 returnlen;
...
int ReadCardIdCommandOfCPU(char* cardId) {
	U8 returnlen;
	char returnBuffer[9];
